...
use super::super::composer::{compose_paragraph, compose_paragraph_with_width, ComposedParagraph};
...
impl LayoutEngine {
    ...
    fn layout_table_cells(
        ...
        for (cell_idx, cell) in table.cells.iter().enumerate() {
            ...
            // compose_paragraph_with_width 가 inner_width 를 사용해 자체 line breaking
            // 을 트리거하고, height_measurer 에서도 같은 helper 를 사용해 측정과
            // layout 결과가 일치하도록 한다.
            let mut composed_paras: Vec<ComposedParagraph> = cell
                .paragraphs
                .iter()
                .map(|p| compose_paragraph_with_width(p, inner_width, styles, self.dpi))
                .collect();
